...
enum Web {
    ...
    @MainActor static let world = WKContentWorld.world(name: "mnml")
...
final class Tab: ObservableObject, Identifiable {
    ...
    func arm(hiding css: String) {
        ...
        } else if Preferences.entitledToPasskeys {
            // A site's passkey request is carried out by mnml itself: WebKit
            // only does that for an app's own domains (see Passkeys.swift).
            // Only in a build Apple's entitlement lets do it: without it the
            // Mac refuses every ceremony, and the relay would stand between
            // the page and a password manager's extension that answers
            // passkeys itself — 1Password's, for one — and fail it.
            controller.addUserScript(
...
final class PageView: WKWebView {
    ...
    /// A key the page sent back unused, offered to mnml's shortcuts first —
...
extension WKWebView {
    ...
    /// otherwise: an .html double-clicked in the Finder, once mnml is the
    ...
    /// JavaScript run in mnml's own world (see Web.world), where its page
