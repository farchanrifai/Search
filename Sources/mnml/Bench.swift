...
final class Bench {
    ...
    /// True while `column` draws a second column off screen.
    static var drawingColumn = false
    ...
    private func handle(_ request: [String: Any], _ given: @escaping ([String: Any]) -> Void) {
        ...
        case "tap":
            ...
            // words. Only on a MNML_PROBE run.
        ...
        case "probe":
            ...
            out["rows"] = Bench.rowFrames.map { key, frame -> String in
                "\(key): \(Int(frame.minX)),\(Int(frame.minY)) \(Int(frame.width))x\(Int(frame.height))"
            }.sorted()
        ...
        case "press":
            ...
            // `key` goes straight to a page instead. Only on a MNML_PROBE run.
        ...
        case "snapshot":
            ...
            guard Store.testing else { answer(["error": "snapshot only works on a --test run"]); return }
        ...
        case "field":
            ...
            // and Core Animation's commit included. Only on a MNML_PROBE run.
        ...
        case "bookmark":
            ...
            // until the run loop rests. Only on a MNML_PROBE run.
        ...
        case "menu":
            ...
            // MNML_PROBE run; nothing is drawn.
        ...
        case "window":
            ...
            guard let item = items.first(where: { $0.title == "mnml" }), let action = item.action else {
                answer(["error": "no mnml item in the Window menu", "items": items.map(\.title)])
                return
            }
        ...
        case "dragrow":
            ...
            // The window's own, for its first click to be a click.
            window.makeKey()
            ...
            func post(_ type: NSEvent.EventType, _ y: CGFloat) {
                ...
                // Straight to the window: a test run is hidden and never comes
                // forward, and events queued to the app never reach it.
        ...
        case "column":
            ...
            // A second column, off screen: it mustn't pass its rows off as
            // the window's to `dragrow`.
            Bench.drawingColumn = true
